...
export async function POST(request: NextRequest, { params }: { params: { agentId: string } }) {
  const agentId = params.agentId; // Define agentId at the top
  // --- BEGIN AGENTDOCK ACCESS AUTHENTICATION ---
  /*
  const expectedApiKey = process.env.AGENTDOCK_ACCESS_KEY;
  if (!expectedApiKey) {
    logger.error(LogCategory.API, 'ChatRoute', 'AGENTDOCK_ACCESS_KEY is not configured on the server.', {
      agentId: agentId,
      path: request.nextUrl.pathname
    });
    return NextResponse.json(
      { error: 'API access key not configured on server.', errorCode: ErrorCode.SERVER_CONFIG_ERROR },
      { status: 500 }
    );
  }
  ...
  if (authHeader && authHeader.startsWith('Bearer ')) {
    providedApiKey = authHeader.substring(7); // Remove 'Bearer '
  }

  if (providedApiKey !== expectedApiKey) {
    logger.warn(LogCategory.API, 'ChatRoute', 'Unauthorized access attempt.', {
      agentId: agentId,
      path: request.nextUrl.pathname,
      reason: providedApiKey ? 'Invalid API key' : 'Missing API key'
    });
    return NextResponse.json(
  ...
  */
  // --- END AGENTDOCK ACCESS AUTHENTICATION ---
